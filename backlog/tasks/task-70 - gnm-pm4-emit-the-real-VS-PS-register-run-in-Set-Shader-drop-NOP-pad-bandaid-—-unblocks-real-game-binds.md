...
- [ ] #3 field→offset layout sourced + cited (OpenOrbis / Mesa sid.h); embedded + ps4-pm4-test Tier B unchanged
<!-- AC:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
DONE 2026-07-12 (feat/task-70 @<prior-history>, merged). Set*Shader now emits REAL register runs from the full VsStageRegisters/PsStageRegisters block (no fake 4-regs+big-NOP). Layout sourced+cited = OpenOrbis GnmDriver.h + Mesa sid.h GFX6 offsets; INDEPENDENTLY CONFIRMED by a research subagent (exact 29/40 dword breakdown). VS(29): SetShReg PGM_LO/HI(=0)@0x48, SetShReg RSRC1/2@0x4A, SetContextReg PA_CL_VS_OUT_CNTL@0x207, SPI_VS_OUT_CONFIG@0x1B1, SPI_SHADER_POS_FORMAT@0x1C3, then IT_NOP<11>=12. PS(40): SetShReg PGM_LO/HI@0x08, RSRC1/2@0x0A, SetContextReg Z/COL_FORMAT@0x1C4, PS_INPUT_ENA/ADDR@0x1B3, PS_IN_CONTROL@0x1B6, BARYC_CNTL@0x1B8, DB_SHADER_CONTROL@0x203, CB_SHADER_MASK@0x8F, IT_NOP<11>. KEY: the trailing IT_NOP<11> is AUTHENTIC retail (the real PS4 GNM capture shows a trailing WriteTrailingNop<11> so sceGnmUpdate*Shader can overwrite in-place) — 'drop NOP-pad' = drop the FAKE 4-regs+zero-state, keep the real register runs + real small NOP. NOT emitted by retail (nor us): RSRC3, user_data/SGPR loads, context preamble. PGM_HI forced 0. FLAG task-53: VS RSRC1 normally mixes shader_modifier (left to caller); apply in HLE if 53 needs it. Verify: gnm+libs 100 pass, clippy 0, fmt clean, gnm Vulkan-free. Combined main gate: 29 suites, oracle 6/6.
<!-- SECTION:NOTES:END -->
