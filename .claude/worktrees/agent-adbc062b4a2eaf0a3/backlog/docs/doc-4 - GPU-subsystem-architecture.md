...
GPU subsystem from today's present-only code (Tier A / phase 1) through the phases
in `decision-3` (Gnm boot+trace → PM4 present/sync → embedded-shader draw → GCN
shaders), so each phase plugs in as an increment rather than a rewrite.

**Ground truth for this doc** (read these, don't re-derive):
`decision-3` (Bloodborne north star, phased PM4 path, ash primary + MoltenVK/Metal
policy, interp-as-oracle); `doc-2` (GPU approach research); `doc-3` (OpenOrbis
...
crates/
  ...
  gnm/             ps4-gnm — NEW. Pure, Vulkan-FREE, headless. The command
    ...
    src/
      ...
      exec.rs          The executor: consumes a decoded stream + the GpuBackend
                       trait, applies state, emits draws/presents/clears. Phase
                       3/3.5/4 grow HERE. Backend-agnostic (takes &mut dyn GpuBackend).
      state.rs         GpuState: the incremental register/binding model (§5).
      shader/
        source.rs      ShaderSource + ShaderProvider trait (§4).
        embedded.rs    embedded-id -> host SPIR-V provider (phase 3.5).
        sb.rs          .sb (OrbShdr) header parse (phase 4 entry; no GCN yet).
      cache/
  ...
  gcn/             ps4-gcn — NEW, but EMPTY until phase 4. GCN ISA decode +
                   disassembler + CPU interpreter (the correctness oracle) and,
                   later, the GCN->SPIR-V recompiler. Depends on ps4-core only.
...
pub trait GpuBackend: Send {
    // ---- presentation (phase 1, exists today, just relocated) ----
    ...
    // ---- resource cache backing (phase 3.5+, see §8) ----
...
- **`present` + the softgpu path exist today** and are the *only* methods phase 1
  needs — so the very first refactor is "make the current display loop call
  `backend.present()`", nothing more. Everything else is a stub until its phase.
...
- **Keep the trait small and grow it per phase.** Add `draw`/`bind_*` when phase
  3.5 needs them, not now. The *concept* (a trait exists, `ps4-gnm` targets it) is
  what must land early; the *method list* accretes with the phases.
...
onto `GpuBackend`. The seam is built so **trace-only (phase 2), present-subset
(phase 3), and full draw (3.5+) are three configurations of one pipeline, not
forks.**

### One pipeline, phase-gated packet handlers

```rust
// ps4-gnm::exec (sketch)

pub struct Executor<'a> {
    state: GpuState,                 // §5: grows per phase
...
impl Executor<'_> {
    pub fn run(&mut self, mem: &dyn VirtualMemoryManager, cb: GuestRange) {
        for pkt in pm4::decode(mem, cb) {           // decode is mode-independent
            if trace_enabled() { log(pm4::trace(&pkt)); }  // phase 2, always on w/ env
            match pkt.op {
                ...
                Op::EventWriteEop(..) if self.mode >= PresentSubset
                    => self.backend.signal_eop(..),          // phase 3
                Op::DrawIndexAuto(..) if self.mode == Draw
                    => self.emit_draw(&pkt),                  // phase 3.5+
                _ => { /* decoded + traced, execution deferred to its phase */ }
            }
        }
    }
}
...
The key property: **decode and trace run in every mode.** Phase 2 ships
`ExecMode::TraceOnly` (no backend touched at all — fully headless). Phase 3 flips
present/sync opcodes on. Phase 3.5 flips `Draw` on. Each phase *adds match arms*;
none rewrites the loop, and each earlier phase's arms keep working. Unknown
opcodes are logged and skipped in all modes (task-21 AC #3).
...
- **Alternative (rejected for now):** move the device onto its own GPU thread and
  have the executor call the backend directly. More like shadPS4's async GPU
...
`run(ring, &[(StreamKind::{Dcb,Ccb,Acb}, GuestRange)])`. Phase 2/3 decode+execute the
DCB only, on the graphics ring, but the DCB+CCB pairing, ACB parallelism, and a
...
into a *host pipeline shader*, with two interchangeable providers behind it, so
phase 3.5 (embedded) does not bake in assumptions phase 4 (GCN) must tear out.
...
pub trait ShaderProvider {
    ...
    /// Err = recognized-but-unsupported (phase-4 "needs GCN" clean defer, AC #3).
    fn resolve(&self, r: &ShaderRef, mem: &dyn VirtualMemoryManager)
        -> Result<Option<HostShader>, ShaderUnsupported>;
}
```

- **Phase 3.5 — `EmbeddedShaderProvider`:** recognizes `Embedded{VS,0}` (fullscreen
  quad) and `Embedded{PS,1}` (R/G export) and returns **hardcoded host SPIR-V**
  (the fixed set from doc-3). No GCN. A draw bound to a `GcnBinary` returns
  `Err(ShaderUnsupported)` → executor logs "needs GCN (phase 4)" and skips the draw,
  never crashes (task-24 AC #3).
- **Phase 4 — `GcnShaderProvider`** (in/over `ps4-gcn`): parses the `OrbShdr`
  `ShaderBinaryInfo` header (magic `"OrbShdr"`, `m_type`, `m_length`, semantic
  tables), decodes GCN, and produces SPIR-V — first via the **CPU interpreter as
  oracle**, later the recompiler, differential-tested against each other
  (`decision-3`). Same `HostShader` output; the executor is unchanged.
- Providers are **chained** (`resolve → None` means "try the next"), so 3.5's
  embedded provider stays live and takes precedence for embedded IDs even after the
  GCN provider lands. The resolved `HostShader` feeds `PipelineKey` →
  `backend.get_or_create_pipeline` (the backend caches pipelines by key).

**What 3.5 must NOT hardcode** (traps for phase 4): the executor must route *all*
shader binds through `ShaderRef`/`ShaderProvider` even in 3.5 — it must not special-
case "there are only two shaders" anywhere except inside `EmbeddedShaderProvider`.
`PipelineKey` must already carry a shader *identity* (hash/addr), vertex-input
layout, and RT format, not a hardcoded pipeline handle — otherwise phase 4's
arbitrary shaders have nowhere to key on. Two further seams the shader path must
leave open (details in §C4, §C8): `HostShader`/`ShaderRef` carry a **HW-stage role**
(LS/HS/ES/GS/VS ≠ logical stage, C8) rather than assuming logical==hardware, and the
binding model is **descriptor-from-memory** (V#/T#/S# resolved from user-SGPR-pointed
memory, C4), not Vulkan-descriptor-set shaped — even though 3.5's embedded shaders are
a fixed HW-VS+PS pair with fixed bindings.
...
bindings) is large. **Do not model it as one giant struct that every phase rewrites.**
Instead: a `GpuState` that is (a) a sparse register file plus (b) a small set of
*derived, typed* views that grow per phase.
...
pub struct GpuState {
    ...
    // typed views, added as phases need them:
    targets: RenderTargets,   // phase 3.5: color RT = the videoout fb at first
    vtx:     VertexBindings,  // phase 3.5
    shaders: BoundShaders,    // phase 3.5: current VS/PS as ShaderRef
    // phase 4 adds: textures, samplers, more RTs, depth, blend, …
}
```

- **`SetContextReg`/`SetShReg` packets just write into the sparse `RegFile`** — that
  handler is written *once* (phase 2 can even trace it) and never changes. New state
  is "understanding more register indices", not "new packet handling".
- **Derived views are lazy and additive.** Phase 3.5 adds `targets`/`vtx`/`shaders`
  by *interpreting* the relevant register ranges at draw time. Phase 4 adds
  `textures` etc. the same way. Adding a view never disturbs existing ones.
...
derived) and means the phase-4 explosion of state is *addition*, not *migration*.
...
- **GCN interpreter as differential oracle (phase 4):** the interpreter and the
...
The `MockBackend` + mock-memory pair is the linchpin: they let phases 2–4's logic
be `cargo test`-covered while the real GPU work stays a small, manually-verified leaf.
...
4. **task-21:** implement `pm4/decode.rs` + `trace.rs` + `ExecMode::TraceOnly`.
   Unit-tested headless. task-22 Tier A produces a trace.
5. **Phase 3:** grow the executor's present/sync arms; add `RunCommandList` to the
   channel; `SubmitAndFlip` → existing present path; EOP → equeue. Present path from
   step 1 is reused, not replaced.
6. **Phase 3.5 (task-24):** add draw arms, `EmbeddedShaderProvider`, the minimal
   `ResourceCache` (vertex/index/constant buffers), `get_or_create_pipeline`. First
   GPU-drawn frame. Softgpu present still works for Tier A guests.
7. **Phase 4:** create `ps4-gcn`, add `GcnShaderProvider`, grow cache to
   textures/RTs, interp-then-recompiler.
...
the subsystem collapse at scale (it is shadPS4/RPCS3's hardest subsystem). It is
...
- **Guest-write hooks in the HLE path (available now, coarse).** Guest GPU memory
  ...
  — this is Kyty's "upload everything every frame" and is the acceptable **phase-3.5
  starting point** because the corpus is tiny. It needs no x86jit change.
...
**Recommended sequence:** phase 3.5 uses **submit-time conservative invalidation**
(re-upload referenced buffers each submit; corpus is one draw, cost is nil). Before
phase 4's texture cache makes that too expensive, land a **watched-data-range
dirty API in x86jit** and switch the cache to poll it. `mprotect` stays a documented
fallback if the x86jit API doesn't materialize.

```rust
// ps4-core (sketch) — the seam the cache polls; impls: x86jit-backed, or
// "everything dirty" (phase 3.5), or mprotect-backed (fallback).
...
- **DirtySource** is a `ps4-core` trait (like `KernelInterface`), impl'd against
  ...
  or by a trivial "always dirty" impl for phase 3.5.
...
CPU-visible readback path below. Phase 4 concern; phase 3.5's only RT is the videoout
framebuffer, already handled by the present path.
...
is built** (phase 4, and only exercised when the policy is on). On the fast default
...
### 8.6 Phase sequencing of the cache

| Phase | Cache scope | Invalidation |
|---|---|---|
| 3 (present) | none new — framebuffer only, present path already handles it (zero-copy import or staging, task-18) | n/a |
| 3.5 (embedded draw) | vertex / index / constant buffers (small, linear) | conservative: re-upload referenced ranges each submit |
| 4 (real shaders) | + textures (**detiled on upload**, §C3), + shader `.sb` binaries, + multiple RTs, host-side RT-as-texture aliasing, opt-in readback | x86jit watched-range DirtySource (or mprotect fallback); host RT resolve always, guest readback only if `ReadbackPolicy` on |
...
rewrite to retrofit. **This section does NOT expand scope** — early phases implement
none of them. Each entry states the *minimal seam to leave now* vs. what to *genuinely
defer*, and cross-references the section it touches. The discipline: don't bake in an
...
This is the backbone of GPU↔CPU and GPU↔GPU ordering. Phase 3 already routes EOP →
equeues (§3, mapping table) — **that must be the thin edge of a coherent sync model,
not a one-off.**
...
- **Seam now (§8):** the `ResourceKey`'s `ResLayout` **already carries a tiling
  ...
  linear`** step (identity for linear layouts). Phase 3.5's buffers are linear so the
  step is a no-op then, but the seam exists.
...
often through a **fetch shader** for vertex attributes. **Trap:** phase 3.5's embedded
shaders have fixed bindings; if the binding model bakes that in, phase 4 tears it out.
...
- **Seam now (§4, §5):** shape the binding abstraction as **descriptor-from-memory**.
  ...
  the `.sb` header). Phase 3.5's `EmbeddedShaderProvider` returns a *fixed* such layout
  — but the executor consumes it through the same memory-driven path, so phase 4's
...
- **Defer:** fetch-shader emulation, actual V#/T#/S# decode, SRT (shader resource
  tables) — all phase-4, inside `ps4-gcn`.
...
- **Seam now (§8):** the resource cache's per-range policy decision must be able to
  ...
  it (even if phase 3.5 always treats everything as copy-side); don't hardcode a
...
counters (`IncrementCeCounter` / `WaitOnDeCounterDiff`). shadPS4's `Liverpool`
processes them concurrently (coroutines). `sceGnmSubmitCommandBuffers` already takes
...
- **Seam now (§3):** the executor's **submission abstraction takes a *set* of
  ...
  stream, and phase-3 execution can run DCB only — but `GnmDriver`/`Executor` must
...
- Sources: shadPS4 DeepWiki "Graphics Command Processing" / "Liverpool Command
  Processor"; GPCS4 `GraphicsStack.md`.
...
- Sources: shadPS4 Liverpool register handling; AMD GCN PM4 register banks.
...
case; Vega **merges LS+HS and ES+GS**. **Trap:** phase 3.5's embedded shaders are a
simple HW-VS+PS pair; baking `logical==hardware` in preludes phase 4.
...
- **Seam now (§4, §7):** `HostShader`/`ShaderRef` and the pipeline derivation carry a
  **HW-stage role** (derived from register state), not a logical stage. The
  ...
  role-carrying seam.
- **Defer:** tessellation/geometry stage handling, LS+HS / ES+GS merging — phase-4,
  inside `ps4-gcn`.
...
- Sources: shadPS4/GPCS4 surface handling; AMD GCN DCC/HTILE docs.
...
deferred and must **not** be built ahead of its phase.

---

## Phase → architecture-seam mapping

| Phase | New/changed code | Seam(s) it touches |
|---|---|---|
| 1 (present, today) | display loop calls `backend.present()` | **`GpuBackend`** (present+import only) |
| 2 — task-20 | `libsceGnmDriver` HLE (libs) → `GnmDriver` (gnm) | HLE glue; `GnmDriver` records submit ranges |
| 2 — task-21 | `pm4/decode.rs`, `trace.rs`; `ExecMode::TraceOnly` | **PM4 decoder** + executor loop (no backend) |
| 2 — task-22 | example ELFs (corpus) | exercises decoder/trace (Tier A), embedded shaders (Tier B) |
| 3 (present/sync) | executor present/sync arms; `RunCommandList` channel msg; EOP→equeue | executor; `GpuBackend::{present,signal_eop}`; **`GpuState` regs** |
| 3.5 — task-24 | executor draw arms; `EmbeddedShaderProvider`; minimal `ResourceCache` | **`ShaderProvider`**, **`ResourceCache`**, `GpuBackend::{draw,bind_*,get_or_create_pipeline}`, `GpuState` views |
| 4 (GCN) | `ps4-gcn` (interp→recompiler); `GcnShaderProvider`; full cache | **`ShaderProvider`** (2nd impl), **`ResourceCache`** (textures/RT/readback), **`DirtySource`** |
...
3. **`ShaderProvider` and `ResourceCache`/`DirtySource` *trait* stubs** (not impls).
   ...
   upload/import surface; defining the seams early keeps 3.5 from hardcoding "two
   shaders" / "no cache".
...
- Command-encoder / render-graph / allocator abstractions — never, unless a real
  backend forces them.
- `ps4-gcn` contents — until phase 4.
- `mprotect` dirty tracking / x86jit watched-range consumption — until phase-3.5's
  conservative invalidation becomes a bottleneck.
- RT readback / RT-as-texture resolve — until a guest exercises them (phase 4).
...
the existing softgpu example. This establishes every seam the phased GPU work plugs
...
1. **Fantasy GPU-HAL.** Do NOT build a portable command-encoder / barrier / render-
   ...
   Vulkan verbs inside the ash impl, and grow the trait one method per phase. One
...
2. **Premature invalidation machinery.** Do NOT build `mprotect` signal handlers or
   a full page-dirty subsystem for phase 3.5. Use conservative "re-upload referenced
   ranges each submit" (the corpus is one draw). Only build the real `DirtySource`
   (ideally an x86jit watched-range API) when phase 4's texture cache needs it.
...
1. **x86jit watched-data-range dirty API (blocking for phase-4 cache).** Confirmed:
...
2. **GPU thread model.** OK to keep the display thread owning the Vulkan device and
   ...
   today's present path), rather than a dedicated async GPU thread (shadPS4-style)?
   The latter is deferred; confirm that's acceptable.
3. **Crate granularity.** Three new crates (`ps4-gnm`, `ps4-gcn`, and the reshaped
   `ps4-gpu`) vs. keeping `ps4-gcn` as a module inside `ps4-gnm` until phase 4. This
   doc recommends a separate `ps4-gcn` crate but empty until phase 4 — acceptable,
   or prefer to defer even the crate until then?
4. **RT readback scope + per-title config.** Do any near-term corpus/target guests
   read a render target back on the CPU, or is readback safely a phase-4-only concern
   ...
   workstream** (mirror RPCS3's "Write Color Buffers" per-game toggle) — confirm the
...
5. **onion/garlic memory-type flag threading (memory-manager ↔ GPU-cache dep, C5).**
   ...
   deferred until phase 4 makes the distinction matter?
...
trait + the phased seam split) records the cross-cutting commitment this doc
implies.*
