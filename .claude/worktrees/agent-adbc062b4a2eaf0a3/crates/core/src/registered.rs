...
pub struct Registered<T: ?Sized> {
    ...
    /// Serializes `override_scoped` callers so concurrent tests don't observe each other's
    /// override; never taken on the hot `register`/`get` path. Compiled in only under the
    /// `test-hooks` feature (or in-crate tests), so it does not leak into plain library builds.
    #[cfg(any(test, feature = "test-hooks"))]
    test_lock: Mutex<()>,
}
