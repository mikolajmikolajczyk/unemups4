...
//! legal constraint). A SELF is the PS4 executable container: an SCE header + a
//! segment table wrapping a plaintext inner ELF. This layer detects the container
//! by magic, reconstructs the inner ELF bytes from the segment table, and
//! **retains** the SCE/SELF container metadata (rather than discarding it) for
//! later layers. It contains no crypto, no keys, and no SAMU logic; encrypted or
//! compressed payloads are rejected with an explicit error rather than any decode
//! attempt.
//!
//! Oracles for the layout constants below. The inner-ELF fields are base ELF64:
//! their struct offsets are witnessed by FreeBSD 9 (Orbis OS base) `sys/elf64.h`
//! `Elf64_Ehdr`/`Elf64_Phdr` and `sys/elf_common.h` `ELFMAG*`. The SELF **outer
//! container** (its magic, header size, segment-table stride, and per-segment
//! flag layout) is not in a base-ELF header; those values are corroborated
//! byte-for-byte against the real dumped eboots at `/home/mikolaj/PS4/{CUSA11302,
//! CUSA05952,CUSA03173}/eboot.bin` (see `container_layout_matches_oracle` and the
//! `real_dumps_extract_inner_elf` smoke test).

use byteorder::{LittleEndian, ReadBytesExt};
use std::io::Cursor;

// Base-ELF magic: bytes 0x7F 'E' 'L' 'F' (FreeBSD 9 sys/elf_common.h ELFMAG0=0x7f,
// ELFMAG1='E', ELFMAG2='L', ELFMAG3='F'), read little-endian.
const ELF_MAGIC: u32 = 0x464c_457f;
// SELF outer-container magic: the first four bytes 0x4F 0x15 0x3D 0x1D observed at
// offset 0 of every real eboot.bin dump (CUSA11302/CUSA05952/CUSA03173), read
// little-endian.
const SELF_MAGIC: u32 = 0x1d3d_154f;

// SCE/SELF header size and per-segment table stride: 0x20 bytes each. Corroborated
// by the real dumps — in each eboot.bin the segment table starts at 0x20 and the
// inner ELF magic lands at 0x20 + num_segments*0x20 (e.g. CUSA11302: 10 segments →
// inner ELF at 0x160). See container_layout_matches_oracle.
const SCE_HEADER_SIZE: usize = 0x20;
const SELF_SEGMENT_SIZE: usize = 0x20;
// Elf64_Ehdr is exactly 0x40 bytes (FreeBSD 9 sys/elf64.h: EI_NIDENT=16 + Half e_type
// + Half e_machine + Word e_version + Addr e_entry + Off e_phoff + Off e_shoff + Word
// e_flags + 6*Half = 0x40).
const ELF_HEADER_SIZE: usize = 0x40;

// SELF per-segment `flags` (first u64 of each 0x20-byte table entry) bit layout.
// Corroborated against the real dumps: in each eboot.bin the entries split into two
// kinds — table entries carrying bit 11 (0x800) set are the ones whose file image
// must be copied to reconstruct the inner ELF, and `flags >> 20` on those yields the
// backing program-header index (CUSA11302: copied entries carry ids 0,1,2,8,9, all
// valid phdr indices; the reconstruction is validated by real_dumps_extract_inner_elf).
// Compression is NOT reliably derivable from a flag bit across dumps (the low flag
// nibble also encodes ordering / self-segment kind), so it is detected the
...
///
/// Kept deliberately minimal (doc-3 open-Q5: grow per-consumer, don't model the
...
pub struct ContainerMeta {
    ...
    /// does not parse. Left `None` until a consumer needs it (doc-3 open-Q5); a
...
/// ones), **not** a plugin registry (doc-3 §5 trap #1):
...
/// Returns `(e_phoff, e_phentsize, e_phnum)`, all little-endian. Field offsets are
/// the `Elf64_Ehdr` layout in FreeBSD 9 `sys/elf64.h` (16-byte e_ident, then Half
/// e_type/e_machine, Word e_version, Addr e_entry, Off e_phoff, Off e_shoff, Word
/// e_flags, Half e_ehsize/e_phentsize/e_phnum/…):
///   e_phoff     @ 0x20 (Off,  u64)
///   e_phentsize @ 0x36 (Half, u16)
///   e_phnum     @ 0x38 (Half, u16)
...
/// `(p_offset, p_filesz)`, both little-endian. Field offsets are the `Elf64_Phdr`
/// layout in FreeBSD 9 `sys/elf64.h` (Word p_type, Word p_flags, Off p_offset, Addr
/// p_vaddr, Addr p_paddr, Xword p_filesz, …; entry size 0x38):
///   p_offset @ 0x08 (Off,   u64)
///   p_filesz @ 0x20 (Xword, u64)
...
fn extract_from_self(raw: &[u8]) -> Result<(Vec<u8>, ContainerMeta), ContainerError> {
    ...
    // num_segments is a u16 at 0x18 in the SCE/SELF header. Corroborated by the real
    // dumps: CUSA11302 reads 0x000A at 0x18 (10 segments) and its inner ELF magic sits
    // at 0x20 + 10*0x20 = 0x160, exactly where this count places it.
    ...
    // Container metadata retained at unwrap (doc-3 L1). Kept minimal: the segment
    // count (already read above) and the SELF header's key_type/category u16 at
    // 0x08 as a coarse program/content-type discriminator (all three reference dumps
    // read 0x0101 there). The finer module attributes live in the not-yet-parsed SCE
    // program-info header -> None.
    ...
    for ph in &phdrs {
        // A single segment's file image cannot legitimately exceed the whole
        // source container: every byte p_filesz declares must be physically
        // present in the SELF we were handed (whether the container packs the
        // segments compactly or preserves their file offsets). Reject a corrupt
        // or crafted p_filesz here, before it inflates the output allocation to
        // an unbacked, resource-exhausting size.
        if ph.file_size > raw.len() as u64 {
            return Err(ContainerError::Malformed(format!(
                "phdr file size {:#x} exceeds source size {:#x}",
                ph.file_size,
                raw.len()
            )));
        }
    ...
    // Allocate the output image fallibly. A corrupt phdr can still drive out_size
    // arbitrarily high through p_offset alone (out_size = max p_offset+p_filesz),
    // and an infallible `vec![0u8; out_size]` would abort the entire emulator on
    // an oversized request. Fail the request with a recoverable container error
    // instead of aborting.
    let mut out: Vec<u8> = Vec::new();
    out.try_reserve_exact(out_size).map_err(|_| {
        ContainerError::Malformed(format!("reconstructed image too large ({out_size} bytes)"))
    })?;
...
mod tests {
    ...
    /// Pins the container layout constants to their oracle values.
    ///
    /// Inner-ELF constants are base ELF64: right-hand literals are the `Elf64_Ehdr`
    /// size and the `ELFMAG*` bytes from FreeBSD 9 (Orbis OS base) `sys/elf64.h` /
    /// `sys/elf_common.h` (`ELFMAG0`=0x7f, `ELFMAG1..3`='E','L','F'; `EI_NIDENT`=16,
    /// so `Elf64_Ehdr` is 0x40 bytes). The SELF outer-container constants (magic,
    /// header size, table stride, flag bits) are the values observed byte-for-byte
    /// in the real dumped eboots — see `real_dumps_extract_inner_elf`.
    #[test]
    fn container_layout_matches_oracle() {
        // Base ELF64 magic = little-endian [0x7f,'E','L','F'] (FreeBSD sys/elf_common.h).
        assert_eq!(
            ELF_MAGIC,
            u32::from_le_bytes([0x7f, b'E', b'L', b'F']),
            "ELF magic must be 0x7F 'E' 'L' 'F' little-endian"
        );
        // Elf64_Ehdr is exactly 0x40 bytes (FreeBSD sys/elf64.h).
        assert_eq!(ELF_HEADER_SIZE, 0x40);

        // SELF outer-container magic = little-endian [0x4F,0x15,0x3D,0x1D], the first
        // four bytes of every reference eboot.bin dump.
        assert_eq!(
            SELF_MAGIC,
            u32::from_le_bytes([0x4f, 0x15, 0x3d, 0x1d]),
            "SELF magic must be 0x4F 0x15 0x3D 0x1D little-endian"
        );
        // SCE header + segment-table stride = 0x20 each (dump-corroborated: inner ELF
        // lands at 0x20 + num_segments*0x20).
        assert_eq!(SCE_HEADER_SIZE, 0x20);
        assert_eq!(SELF_SEGMENT_SIZE, 0x20);

        // SELF per-segment flag layout (dump-corroborated): blocked bit 11 marks the
        // entries copied to rebuild the ELF; `flags >> 20` (12-bit) is the phdr index.
        assert_eq!(SELF_SEG_BLOCKED_BIT, 1 << 11);
        assert_eq!(SELF_SEG_ID_SHIFT, 20);
        assert_eq!(SELF_SEG_ID_MASK, 0xFFF);

        // Concrete decode of CUSA11302's first blocked table entry (flags = 0x2804
        // observed at offset 0x40): bit 11 set (copied), id 0 (backs phdr 0).
        let flags: u64 = 0x2804;
        assert_ne!(
            flags & SELF_SEG_BLOCKED_BIT,
            0,
            "0x2804 has the blocked bit set"
        );
        assert_eq!((flags >> SELF_SEG_ID_SHIFT) & SELF_SEG_ID_MASK, 0);
    }
    ...
    fn build_fake_self(inner_elf: &[u8], encrypted: bool, compressed: bool) -> Vec<u8> {
        ...
        // Offsets are Elf64_Ehdr fields (FreeBSD 9 sys/elf64.h): e_shoff @ 0x28 (Off),
        // e_shnum @ 0x3C (Half).
    ...
    #[test]
    fn self_phdr_filesz_exceeding_source_is_malformed() {
        // A corrupt/crafted inner ELF whose program header declares a file image
        // larger than the whole SELF must be rejected as malformed, not turned
        // into a multi-terabyte output buffer that aborts the process on alloc.
        let elf = load_example_elf();
        let mut fake = build_fake_self(&elf, false, false);
        // The reconstructed inner ELF header/phdr table sits immediately after the
        // SELF segment table. Patch phdr 0's p_filesz (Elf64_Phdr p_filesz @ 0x20,
        // FreeBSD 9 sys/elf64.h) to a value far larger than the container.
        let (e_phoff, _e_phentsize, _e_phnum) = phdr_table_info(&elf);
        let num_segments = u16::from_le_bytes([fake[0x18], fake[0x19]]) as usize;
        let elf_base = SCE_HEADER_SIZE + num_segments * SELF_SEGMENT_SIZE;
        let p_filesz_at = elf_base + e_phoff as usize + 0x20;
        let huge: u64 = 0x0000_7FFF_FFFF_F000;
        fake[p_filesz_at..p_filesz_at + 8].copy_from_slice(&huge.to_le_bytes());
        let err = open(fake).expect_err("oversized p_filesz must be rejected");
        assert!(matches!(err, ContainerError::Malformed(_)), "got {err:?}");
        assert!(err.to_string().contains("exceeds source size"), "{err}");
    }
