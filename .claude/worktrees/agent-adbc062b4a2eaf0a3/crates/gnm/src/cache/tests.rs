//! Headless cache unit tests (doc-4 §6, §8.4): the cache emits a `Vec<BackendCmd>`
...
fn first_use_clean_reuse_dirty_reupload() {
    ...
    let k_a = key(0x1000, 0x100, ResLayout::VertexBuf);
...
fn drain_marks_only_overlapping_entries() {
    ...
    let k_c = key(0x5000, 0x100, ResLayout::ConstBuf); // far away — no overlap
...
/// desktop-with-extension mirror that reaches the import fork (doc-4 §C5).
struct AllGarlic;
...
/// An import mirror that accepts every candidate range (doc-4 §8.2 desktop mirror).
...
/// (doc-4 §C5). The default cache never imports even when the range would be a
...
fn first_use_failed_read_inserts_dirty_and_retries() {
    ...
    let k = key(0x1000, 0x100, ResLayout::ConstBuf);
...
fn free_evicts_only_overlapping_entries() {
    ...
    let k_a = key(0x1000, 0x100, ResLayout::VertexBuf); // [0x1000, 0x1100)
    let k_c = key(0x5000, 0x100, ResLayout::ConstBuf); // far away
...
// ---- sampled-texture cache path (doc-4 §C3/§C4) ----------------------------

use ps4_core::gpu::{SamplerAddressMode, SamplerDesc, SamplerFilter, TextureFormat};
