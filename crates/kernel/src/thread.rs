...
impl Thread {
    pub fn execute(self: &Arc<Self>) -> std::io::Result<()> {
        ...
        let handle = builder.spawn(move || {
            ...
            let mut top = stack_top & !0xF;

            // Main thread only: build the Orbis initial argument block near the top of the
            // stack and pass its address in RDI. The eboot CRT entry reads `argc = *(rdi)`
            // and `argv = rdi + 8` (verified by disassembly of the entry at e_entry). Handing
            // it an unconstructed stack made `*(rdi)` a garbage argc: Mono sized its argv copy
            // as `g_malloc((argc+1)*8)` — which failed for the huge junk count, leaving its
            // saved-argv global NULL — then crashed on `*(NULL) = strdup(argv[0])`, with
            // `dirname(argv[0]=NULL)` asserting `filename != NULL` on the way. argv[0] is the
            // program path so Mono's `dirname(argv[0])` resolves the assembly base to /app0
            // (the dump mount root).
            let rdi_main = if this.is_main {
                let path = b"/app0/eboot.bin\0";
                top -= path.len() as u64;
                let str_addr = top;
                let _ = vm.write_bytes(str_addr, path);
                top &= !0xF;
                // argc, argv[0], argv[1]=NULL, envp[0]=NULL, auxv AT_NULL (type,val).
                let words: [u64; 6] = [1, str_addr, 0, 0, 0, 0];
                top -= (words.len() * 8) as u64;
                top &= !0xF;
                let block_addr = top;
                let mut buf = [0u8; 48];
                ...
                let _ = vm.write_bytes(block_addr, &buf);
                Some(block_addr)
            } else {
                None
            };

            // Scratch stack pointer, below the arg block (main) or below the stack top
            // (workers). The guest only touches addresses <= start_rsp, so the arg block
            // above it is safe.
            let mut rsp = top;
            rsp &= !0xF;
            rsp -= 8;
            ...
            // Main thread: RDI = the arg-block pointer built above. Worker threads: RDI = the
            // entry argument. FS base = the thread's guest TLS base, installed directly into
            // the Vcpu's `Reg::FsBase` — the host FS is untouched, so Rust TLS keeps working
            //.
            let rdi = rdi_main.unwrap_or(this.entry_argument);
...
impl ThreadManager {
    ...
    pub fn create_thread(
        ...
        if let Some(data) = tls_init_data {
            ...
            // zero tbss (after data, before padding). `saturating_sub`: a malformed PT_TLS with
            // p_memsz < p_filesz (mem_size < data.len()) would otherwise underflow — release
            // wraps to a huge size that fails `zero_memory`, debug panics — crashing thread
            // startup on a corrupt binary. Clamp to 0 (no tbss) so bring-up fails cleanly.
            let tbss_size = tls_data_size.saturating_sub(data.len());
            if tbss_size > 0 {
                memory
    ...
    /// The debug name of a live thread, for host-side reports (profiler dump, fault
    /// annotator). `None` for a tid that has exited or was never created here.
    pub fn thread_name_of(&self, tid: u32) -> Option<String> {
        let threads = self.threads.read().ok()?;
        let t = threads.get(&tid)?;
        let name = t.name.lock().ok()?.clone();
        Some(name)
    }
    ...
    pub fn thread_get_name(
        ...
        // A zero-length buffer has no room even for the trailing NUL. Guard the
        // empty case up front: `len - 1` on a guest-controlled `len == 0` would
        // underflow to usize::MAX, making `min(..)` select the full name and
        // clobber memory past the caller's buffer. Nothing to write, so report
        // success without touching guest memory.
        if len == 0 {
            return Ok(0);
        }
