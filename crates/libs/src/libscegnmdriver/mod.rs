//! libSceGnmDriver HLE (doc-2 §1). Gnm/Gnmx are statically linked into
...
//!
//! These handlers are *thin* (doc-2 §1): the submit stubs extract the DCB/CCB
...
//! organizational:
//!
//! - [`submit`] — submit / submit-and-flip / submit-done + the workload submit variants
//!   (`Submit{,AndFlip}CommandBuffersForWorkload`) + guest-array readers.
//! - [`workload`] — the workload-stream lifecycle (`Create/Destroy/Begin/End`, `DingDong`,
//!   `RequestFlipAndSubmitDone`) — bookkeeping around the same submit path (doc-6 Entry 1).
//! - [`draw`] — draw / dispatch / compute-queue command builders.
//! - [`shader_bind`] — embedded-shader and `sceGnmSet*Shader` register binds.
//! - [`hwstate`] — HW-state / context-state-init preambles, VGT control, submit gating,
//!   the command-buffer validation getters, markers, cache flush.
//!
//! The event-queue GPU-completion glue (`sceGnmAddEqEvent` / `DeleteEqEvent` /
//! `GetEqEventType`) lives in [`crate::libkernel::events`] next to the equeue it drives.
...
mod tests {
    ...
    /// task-138: a host-backed bounded-read source (guest ptr == host addr) that copies
    /// straight from host memory. The submit-array readers now route through the bounded
    /// seam (no raw-identity fallback), so these tests wire this to feed their host-array
    /// DCB/CCB pointer/size blocks. Read-only; only `read_ranged` is meaningful.
    struct HostMem;
    impl BoundedRead for HostMem {
    ...
    fn submit_handler_records_ranges_in_driver() {
        ...
        // Wire the host-backed bounded-read seam so the submit-array readers can pull the
        // host-array pointer/size blocks (guest ptr == host addr).
        let _rd = registered_source().override_scoped(Arc::new(HostMem) as Arc<dyn BoundedRead>);
