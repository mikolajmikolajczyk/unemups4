...
// (real GNM, and shadPS4's PatchFlipRequest) REQUIRES the packet to be present —
...
// pure NOP to the GPU: unemups4's PM4 executor and shadPS4's Liverpool both skip
...
int main(void) {
    ...
    *cmd++ = 0;               // draw initiator flags
    ...
    // submit-and-flip (shadPS4) shows nothing if the guest falls off the end
    ...
    for (;;) {
        ...
        // (shadPS4) consumes the request by PATCHING the tag in place — after a
        ...
        // unemups4 and shadPS4 honor; it also yields so the display thread can
