...
- `GpuState { ctx_regs, sh_regs, uconfig_regs, config_regs: RegFile, shaders: BoundShaders }`. Derived-view `BoundShaders` kept as a FIELD (doc-4 §5). Methods: `apply_set_reg(base, body)` (body = [offset, v0, v1…] → writes v_i at base+offset+i; empty/offset-only body = no-op), `clear_regs()` (all four banks; shader view left intact), `bind_embedded_shader(stage,id)`.
