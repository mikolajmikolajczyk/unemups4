...
the phase-2 PM4 work directly against that would calcify raw `ash` across the
command processor and force a rewrite at each phase.

`doc-4 - GPU-subsystem-architecture.md` designs the target architecture and the
...
2. **The command processor lives in a Vulkan-free `ps4-gnm` crate; GCN shader
   translation in a separate `ps4-gcn` crate (empty until phase 4).** Dependency
...
3. **One pipeline, phase-gated.** PM4 decode + trace run in every mode; phases
   ...
   `ShaderProvider` seam (embedded-id → hardcoded SPIR-V in phase 3.5; GCN `.sb` →
   SPIR-V via interp-then-recompiler in phase 4) so 3.5 hardcodes nothing phase 4
   must tear out.
...
- **Dedicated async GPU thread (shadPS4-style) now.** Deferred: the display
...
- **Reuse x86jit SMC dirty tracking for cache invalidation as-is.** Not possible
  ...
  conservative per-submit re-upload as the phase-3.5 stand-in and `mprotect` as a
  documented fallback.
...
- New cross-crate dependency surfaced: a memory-type (onion/garlic) flag threaded
  ...
  dirty API. Both recorded as open questions in `doc-4`.
...
educational Tier A/B, `ps4-gcn` and the heavier cache/constraint seams can be
