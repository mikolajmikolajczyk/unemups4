...
pub trait ExecutableImage: Send + Sync {
    ...
    /// The modules (`.prx` files) that must be mapped and have their exports registered
    /// BEFORE this image is relocated, as opposed to [`Self::libraries`], which names the
    /// import namespaces its symbols resolve against.
    ///
    /// Identical for standard ELF, where a `DT_NEEDED` string is the file name; different in
    /// the SCE format, where module and library are separate namespaces. Defaults to
    /// `libraries()` so a non-SCE image needs no impl.
