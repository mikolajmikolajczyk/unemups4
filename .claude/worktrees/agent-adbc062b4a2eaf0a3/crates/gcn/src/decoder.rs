//! GCN instruction-stream decoder (doc-4 §1, phase 4).
//!
...
pub fn decode_one(words: &[u32]) -> Decoded {
    ...
    // formats share the 0b10 top prefix and are distinguished by more bits.
    if w0 >> 26 == 0b111110 {
...
fn decode_scalar(words: &[u32]) -> Decoded {
    ...
    // SOP1: [31:23] = 0b10_1111101 (0xBE prefix byte). Fields: SSRC0[7:0],
    // OP[15:8], SDST[22:16].
    ...
    // SOP2: [31:30] = 0b10 (fall-through for the scalar space).
...
fn decode_vop3(words: &[u32]) -> Decoded {
    let w0 = words[0];
    // VOP3 is always two dwords. A missing second dword decodes what it can.
    let w1 = words.get(1).copied().unwrap_or(0);
    let op = ((w0 >> 17) & 0x1FF) as u16;
    let vdst = Operand::Vgpr((w0 & 0xFF) as u8);
    let abs = ((w0 >> 8) & 0x7) as u8;
    // omod occupies bits [28:27] of the high dword (just below neg[31:29]).
...
fn decode_mubuf(words: &[u32]) -> Decoded {
    ...
    // MUBUF SOFFSET is an 8-bit field: an SGPR, an inline constant, or m0 — GFX7
    // does NOT permit a 32-bit literal here (llvm-mc rejects one). Field 255 is
    // therefore invalid; keep it as a raw marker rather than fabricating a
    // `Literal(0)`, which would (wrongly) imply a trailing dword and desync the PC.
    // MUBUF is unconditionally two dwords regardless.
...
fn decode_mimg(words: &[u32]) -> Decoded {
    // MIMG is unconditionally two dwords. Field layout (GFX7, verified against llvm-mc):
    //   w0: dmask[11:8], unrm[12], glc[13], da[14], r128[15], tfe[16], lwe[17], op[24:18]
    //   w1: vaddr[7:0], vdata[15:8], srsrc[20:16] (×4 SGPR), ssamp[25:21] (×4 SGPR)
    let w0 = words[0];
    let w1 = words.get(1).copied().unwrap_or(0);
    let op = ((w0 >> 18) & 0x7F) as u8;
    ...
    let vaddr = Operand::Vgpr((w1 & 0xFF) as u8);
    let vdata = Operand::Vgpr(((w1 >> 8) & 0xFF) as u8);
    // The T#/S# register fields count in units of 4 SGPRs (like MUBUF's srsrc).
    let srsrc = (((w1 >> 16) & 0x1F) * 4) as u8;
