...
SPEC = doc-3 (loader architecture) steps L1 + L2 + the L3 trait stub; migration step 1 (pure refactor). Read doc-3 first — it is the loader design, mirroring doc-2 for GPU. Maintainer decisions 2026-07-10: form (A) — keep the ExecutableImage trait, back it with the cache (smallest diff, callers unchanged), NOT form (B) collapse.
