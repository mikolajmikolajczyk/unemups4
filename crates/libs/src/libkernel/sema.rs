...
impl HostSem {
    ...
    /// Wait for `need` permits, giving up after `timeout`; `true` if the permits were
    /// taken, `false` on timeout (task-216).
    ///
    /// The deadline is fixed at entry and not recomputed per wakeup: `Condvar::wait_timeout`
    /// can return spuriously, and restarting the timeout each time would let a semaphore
    /// under steady contention wait indefinitely — the failure the timeout exists to prevent.
    fn wait_timeout(&self, need: i64, timeout: Duration) -> bool {
        ...
        while *count < need {
            ...
            let (new_count, result) = self.cond.wait_timeout(count, deadline - now).unwrap();
            count = new_count;
            // Do not trust `timed_out()` alone: a post can land in the same instant the
            // timeout fires, and the permit is then ours to take. Re-check the count.
            if result.timed_out() && *count < need {
                return false;
            }
        }
        *count -= need;
...
pub fn sys_sem_trywait(sem: u64) -> i32 {
    if posix_get_or_create(sem, 0).try_wait(1) {
        ...
        // Unavailable permit: FreeBSD sem_trywait returns -1 with errno=EAGAIN (35).
        // The errno MUST be set — Mono's mono_os_sem_timedwait polls sem_trywait and
        // g_error/aborts if the failure errno isn't EAGAIN (it read a stale 0 =
        // "No error" and killed the thread, deadlocking whoever waited on it).
        ps4_cpu::set_errno(35);
        -1
    }
}

/// POSIX `sem_timedwait(sem, const struct timespec *abstime)` — an ABSOLUTE deadline.
///
/// This used to ignore the deadline and block exactly like `sem_wait`. The comment defending
/// that argued it was "correct for a permit that eventually arrives", which is precisely the
/// case where the timeout does not matter; it said nothing about the case the API exists for.
/// A guest that uses the timeout to make progress — to poll, to back off, to report a stall —
/// instead hung forever (task-216).
///
/// FreeBSD returns -1 with `errno = ETIMEDOUT` on expiry. The errno MUST be set: Mono's
/// `mono_os_sem_timedwait` inspects it and `g_error`/aborts on anything unexpected, which is
/// how the sibling `sem_trywait` bug killed a thread by leaving a stale 0 behind.
#[ps4_syscall(id = SyscallId::SYS_SEM_TIMEDWAIT, lib = crate::libs::LIB_KERNEL, names = ["sem_timedwait", "_sem_timedwait"])]
pub fn sys_sem_timedwait(sem: u64, abstime: u64) -> i32 {
    let micros = match super::pthread::abstime_to_relative_micros(abstime) {
        Ok(us) => us,
        Err(errno) => {
            ps4_cpu::set_errno(errno);
            return -1;
        }
    };
    if posix_get_or_create(sem, 0).wait_timeout(1, Duration::from_micros(micros as u64)) {
        0
    } else {
        ps4_cpu::set_errno(60); // ETIMEDOUT (FreeBSD; Linux's 110 makes Mono abort)
        -1
    }
}
...
pub fn sce_kernel_create_sema(
    ...
    // task-115: validate the out-param through the GuestPtr seam before touching it; a junk
    // pointer fails the constructor (out of arena) and returns EINVAL instead of segfaulting.
    let Some(out) = GuestPtr::<u32>::new(out_id as u64) else {
...
/// `sceKernelWaitSema(sema, need, OrbisKernelUseconds *timeout)` — a POINTER to a RELATIVE
/// microsecond count, per `data/oo_sdk/include/orbis/libkernel.h:449`. NULL means wait
/// forever, which is the only case this used to implement (task-216).
///
/// A non-null pointer outside the guest arena falls back to the infinite wait rather than
/// dereferencing it or failing the call: the permit is what the guest is really after, and a
/// bad timeout pointer is not worth turning into a spurious error on a path Celeste takes
/// thousands of times per run.
#[ps4_syscall(id = SyscallId::SCE_KERNEL_WAIT_SEMA, lib = crate::libs::LIB_KERNEL, name = "sceKernelWaitSema")]
pub fn sce_kernel_wait_sema(id: u32, need: i32, timeout: u64) -> i32 {
    let micros = if timeout != 0 && crate::is_guest_range(timeout, size_of::<u32>() as u64) {
        // Identity-mapped guest pointer (doc-2 §1), range-checked above.
        Some(unsafe { *(timeout as *const u32) })
    } else {
        None
    };
    ...
    match sem {
        Some(s) => {
            ...
            match micros {
                Some(us) => {
                    if s.wait_timeout(need, Duration::from_micros(us as u64)) {
                        0
                    } else {
                        0x8002003Cu32 as i32 // SCE_KERNEL_ERROR_ETIMEDOUT
                    }
                }
                None => {
                    s.wait(need);
