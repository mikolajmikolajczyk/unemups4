      ...
      in
      {
        devShells.default = pkgs.mkShell {
          packages = [
            ...
            pkgs.rustfmt
            pkgs.clippy
            # Layer-boundary enforcement (task-132): `cargo deny` reads deny.toml
            # (intent doc); the authoritative per-edge check is
            # scripts/check-layer-bans.sh, which needs `jq`.
            pkgs.cargo-deny
            pkgs.jq
          ...
          env.LD_LIBRARY_PATH = pkgs.lib.makeLibraryPath [
            ...
            pkgs.libxkbcommon
            pkgs.vulkan-loader
            # cpal links libasound at link-time (NEEDED); without it on the
            # runtime path the emulator won't even start (audio bring-up).
            pkgs.alsa-lib
            # gilrs (gamepad input) links libudev.so.1 at link-time.
            pkgs.udev
          ];
        };
      }
