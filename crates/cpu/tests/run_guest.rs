...
const SYS_NULLRET: u64 = 105; // task-113.2: a pointer-returning HLE that answers NULL
    ...
    match id {
        SYS_ARGS => {
            *captured().lock().unwrap() = Captured {
                ...
                rsi: ctx.arg1(),
                rdx: ctx.arg2(),
                rcx: ctx.arg3(), // 4th arg — carried in R10 across the SYSCALL
                r8: ctx.arg4(),
                r9: ctx.arg5(),
            };
            ARGS_RET
        ...
        // The shape task-113.2's breadcrumb exists to catch: a handler that hands the
        // guest a NULL where it expects a real pointer and never null-checks.
        SYS_NULLRET => 0,
        SYS_NESTED => {
...
fn ac_g_null_deref_report() {
    ...
    // The faulting CODE is annotated too, not just the (useless, always-0) faulting
    // address — that is what names the module/function for a null deref (task-113.2).
    assert!(
        report.contains("faulting code:"),
        "faulting code annotated: {report}"
    );
}

// --- HLE breadcrumb ring reaches the fault report (task-113.2) ------

#[test]
fn ac_j_fault_report_carries_the_hle_breadcrumb() {
    ensure_dispatch();
    ensure_annotator();
    let (_guard, vm) = make_vm();

    // A guest that calls a pointer-returning HLE which answers NULL, then dereferences
    // the result without checking — the Celeste-CLIMB crash shape in miniature:
    //   bf 34 12 00 00              mov edi, 0x1234        ; an arg to see in the dump
    //   b8 69 00 00 00              mov eax, 105           ; SYS_NULLRET
    //   0f 05                       syscall                ; -> RAX = 0
    //   48 8b 04 25 00 00 00 00     mov rax, qword ptr [0] ; deref the NULL
    vm.write_bytes(
        CODE,
        &[
            0xBF, 0x34, 0x12, 0x00, 0x00, 0xB8, 0x69, 0x00, 0x00, 0x00, 0x0F, 0x05, 0x48, 0x8B,
    ...
    let exit = run_guest_call(&vm, CODE, STACK_TOP, 0, 0, 0);
    let GuestExit::Fatal(report) = exit else {
        panic!("expected Fatal from the null deref, got {exit:?}");
    };

    assert!(
        report.contains("HLE calls on this thread"),
        "breadcrumb block present: {report}"
    );
    assert!(
        report.contains("0x1234"),
        "the call's arguments are recorded: {report}"
    );
    assert!(
        report.contains("-> 0x0"),
        "the NULL return is recorded — the whole point: {report}"
    );
}
