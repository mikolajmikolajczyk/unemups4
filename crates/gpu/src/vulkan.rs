...
pub struct VulkanContext {
    ...
    /// Queryable device capabilities, populated once at device selection from the
    /// REAL device (features/limits + `VK_KHR_portability_subset` when present),
    /// never from `#[cfg(target_os)]` (task-136). This is the seam for a future
    /// caps-tiered Vulkan/MoltenVK split; nothing gates on it yet (behavior is the
    /// portable baseline everywhere). See [`ps4_core::gpu::GpuCaps`].
    pub caps: GpuCaps,
}
...
/// Pick the swapchain surface format, preferring an 8-bit `_SRGB` surface so the present's
/// LINEAR output is hardware-encoded to sRGB on store (linear-correct compositing reaches
/// the display correctly gamma'd; task-154 residual #2). Both channel orders are accepted
/// (the present shader's `swap_rb` handles order independently). Falls back to the driver's
/// first advertised format when no 8-bit `_SRGB` surface is offered — the present shader
/// then encodes linear->sRGB itself (gated by the `srgb` push constant). `_SRGB` swapchains
/// are core Vulkan and MoltenVK-supported (task-133). Returns `None` for an empty format
/// list (a non-conformant/software surface, or a surface-lost race after support was
/// verified) so the caller returns a clean error instead of indexing an empty slice.
...
impl VulkanContext {
    pub unsafe fn new(window: &Window) -> VkResult<Self> {
        ...
        // task-136: query real device capabilities (features/limits +
        // VK_KHR_portability_subset when advertised) into a Vulkan-free GpuCaps.
        // Populated from the actual device, never #[cfg(target_os)] — platform !=
        // capability. Seam only: nothing gates on it yet (baseline unchanged).
        let caps = unsafe { Self::query_caps(&instance, pdevice, &surface_loader, surface) };
        ...
        tracing::info!(
            "GpuCaps: portability_subset={} (triangle_fans={}, separate_stencil_mask_ref={}, \
             image_view_format_reinterpretation={}, point_polygons={}), \
             surface_image_count=[{}..={}]; seam only — baseline behavior unchanged (task-136)",
            caps.portability_subset,
            caps.triangle_fans,
            caps.separate_stencil_mask_ref,
            caps.image_view_format_reinterpretation,
            caps.point_polygons,
            caps.surface_min_image_count,
            caps.surface_max_image_count,
        );
    ...
    unsafe fn create_instance(window: &Window) -> VkResult<(Entry, Instance)> {
        unsafe {
            ...
            // The recompile/validation path is pinned to a Vulkan 1.1 portability
            // floor (decision-3: recompiled SPIR-V never exceeds vulkan1.1).
            // Requesting 1.3 fails on older MoltenVK/loader stacks that only expose
            // ~1.2, so clamp to what the loader reports, capped at 1.2.
            let driver_version = entry
                .try_enumerate_instance_version()?
                .unwrap_or(vk::API_VERSION_1_0);
            let api_version = driver_version.min(vk::API_VERSION_1_2);

            // Newer Vulkan loaders refuse to enumerate a MoltenVK (portability)
            // device unless the instance opts into portability enumeration via
            // VK_KHR_portability_enumeration + the ENUMERATE_PORTABILITY flag. Add
            // both only when the loader advertises the extension, so native
            // Linux/desktop Vulkan is unaffected.
            let instance_exts = entry.enumerate_instance_extension_properties(None)?;
            let portability_enum_name = ash::khr::portability_enumeration::NAME;
            let has_portability_enum = instance_exts.iter().any(|e| {
                std::ffi::CStr::from_ptr(e.extension_name.as_ptr()) == portability_enum_name
            });
            let mut flags = vk::InstanceCreateFlags::empty();
            if has_portability_enum {
                extensions.push(portability_enum_name.as_ptr());
    ...
    /// Query the picked physical device into a Vulkan-free [`GpuCaps`] (task-136).
    ///
    /// The capability decision reads the REAL device — base features, surface
    /// limits, and, when the device advertises `VK_KHR_portability_subset`
    /// (MoltenVK), the `VkPhysicalDevicePortabilitySubsetFeaturesKHR` restriction
    /// bits. There is deliberately NO `#[cfg(target_os)]` here: platform !=
    /// capability (MoltenVK gains features over time; Linux drivers vary), so the
    /// only correct signal is what the device advertises. When the extension is
    /// absent (native desktop Vulkan, a feature superset), the portability flags
    /// stay at their "full" defaults ([`GpuCaps::FULL`]).
    ///
    /// Seam only: the result is threaded to the backend and made available, but no
    /// behavior gates on it yet — the portable baseline (task-133 clamp) runs
    /// everywhere. A caps-tiered fast path is a later, *measured* optimization.
    unsafe fn query_caps(
        ...
        unsafe {
            // Does the device advertise the portability subset? (MoltenVK does;
            // native desktop drivers never do.)
            let portability_subset = instance
                .enumerate_device_extension_properties(pdevice)
                .map(|available| {
                    available.iter().any(|e| {
                        // SAFETY: `extension_name` is a NUL-terminated fixed-size C
                        // string populated by the driver.
            ...
            // Base features via the plain (non-chained) query — always valid.
            let features = instance.get_physical_device_features(pdevice);

            // Portability-subset feature bits: chain the KHR struct into features2
            // ONLY when the extension is present. Off the portability path the
            // struct would be untouched by the driver, so we start it "supported"
            // (all-true) and let the query overwrite it when present — matching the
            // "native = full superset" default in GpuCaps.
            let mut ps_feats = vk::PhysicalDevicePortabilitySubsetFeaturesKHR::default();
            if portability_subset {
                let mut feats2 = vk::PhysicalDeviceFeatures2::default().push_next(&mut ps_feats);
                instance.get_physical_device_features2(pdevice, &mut feats2);
            } else {
                // Native Vulkan: nothing is restricted. Present these as supported.
                ps_feats.triangle_fans = vk::TRUE;
                ps_feats.separate_stencil_mask_ref = vk::TRUE;
                ps_feats.image_view_format_reinterpretation = vk::TRUE;
                ps_feats.image_view_format_swizzle = vk::TRUE;
                ps_feats.constant_alpha_color_blend_factors = vk::TRUE;
                ps_feats.point_polygons = vk::TRUE;
            }

            // Surface image-count limits: the swapchain path that hit
            // VUID-...-presentMode-02839 (min 2 vs required 3) is exactly what a
            // portability-aware sizing path would consult. Best-effort — 0 when the
            // surface query fails.
            let (surface_min_image_count, surface_max_image_count) = surface_loader
                .get_physical_device_surface_capabilities(pdevice, surface)
                .map(|c| (c.min_image_count, c.max_image_count))
                .unwrap_or((0, 0));

            GpuCaps {
                portability_subset,
                triangle_fans: ps_feats.triangle_fans == vk::TRUE,
                separate_stencil_mask_ref: ps_feats.separate_stencil_mask_ref == vk::TRUE,
                image_view_format_reinterpretation: ps_feats.image_view_format_reinterpretation
                    == vk::TRUE,
                image_view_format_swizzle: ps_feats.image_view_format_swizzle == vk::TRUE,
                constant_alpha_color_blend_factors: ps_feats.constant_alpha_color_blend_factors
                    == vk::TRUE,
                point_polygons: ps_feats.point_polygons == vk::TRUE,
                surface_min_image_count,
                surface_max_image_count,
    ...
    unsafe fn create_device(
        ...
        unsafe {
            ...
            // A portability (MoltenVK) physical device advertises
            // VK_KHR_portability_subset, which the spec then REQUIRES be enabled at
            // device creation. Native desktop drivers never advertise it, so this is
            // a no-op off the portability path.
            let portability_subset_name = ash::khr::portability_subset::NAME;
            if has_ext(portability_subset_name) {
                device_extensions.push(portability_subset_name.as_ptr());
            }
    ...
    unsafe fn create_swapchain(
        ...
        unsafe {
            ...
            // Prefer an 8-bit _SRGB surface: presenting an _SRGB swapchain makes the
            // hardware encode the present shader's LINEAR output to sRGB on store, so the
            // linear-space compositing (sRGB-decoded texture samples blended in an _SRGB
            // render target, task-154 residual #2) reaches the display correctly gamma'd
            // with no in-shader encode. Fall back to the driver's first format when no
            // _SRGB surface is offered (the present shader then encodes in-shader, gated by
            // the `srgb` push constant). `choose_surface_format` returns the pick.
            let format =
                choose_surface_format(&formats).ok_or(vk::Result::ERROR_INITIALIZATION_FAILED)?;
            ...
            if extent.width == u32::MAX || extent.width == 0 {
                ...
                extent.width = 1920;
                extent.height = 1080;
                // Clamp the forced size into the surface's allowed range: a compositor that
                // reports the "you choose the size" sentinel may still cap max_image_extent
                // below 1920x1080 (e.g. a 1280x720 window), and an extent outside
                // [min_image_extent, max_image_extent] is rejected by vkCreateSwapchainKHR
            ...
            // One more image than the surface demands, so a frame that overruns a refresh
            // period has somewhere to render while the display still owns the front image.
            // At the surface minimum the presentation engine holds one of our two images
            // for a whole period, `acquire_next_image` blocks on it, and the achieved rate
            // collapses onto an integer division of the refresh rate (task-204).
            ...
            // MAILBOX replaces a queued-but-unpresented image instead of blocking on the
            // queue, and unlike IMMEDIATE it still presents only on vblank — so it removes
            // the acquire stall without introducing tearing. FIFO is the only mode the spec
            // guarantees, so it stays the fallback (and is what a surface reporting nothing
            // else gets).
            ...
            tracing::debug!(
                "swapchain: {} images (surface min {} max {}), present mode {:?}",
                min_image_count,
            ...
            // COLOR_ATTACHMENT is the only swapchain usage the spec guarantees. TRANSFER_SRC
            // (needed only by the env-gated PNG dump, UNEMUPS4_DUMP_PNG, to copy the presented
            // image back to a host buffer) is requested only when the surface actually reports
            // it in supportedUsageFlags — some MoltenVK/Wayland configs omit it (permitted by
            // spec), and requesting an unsupported usage fails vkCreateSwapchainKHR
            // (VUID-VkSwapchainCreateInfoKHR-imageUsage), which would stop the emulator from
            // starting. Where it is absent the PNG dump simply no-ops.
    ...
    unsafe fn create_pipeline(
        ...
        unsafe {
            ...
            // Two u32 FRAGMENT push constants (swap_rb, srgb) the present frag shader reads
            // to R↔B-swap / flag the sRGB scanout, decoded per-flip from the guest scanout
            // pixelFormat (task-154 residual #2).
    ...
    unsafe fn create_texture(
        ...
        unsafe {
            ...
            // _UNORM, NOT _SRGB (task-175): the format must mirror the guest's
            // `CB_COLOR0_INFO.NUMBER_TYPE` for the videoout target, and every title in the
            // corpus programs NUMBER_UNORM (Celeste: `CB_COLOR0_INFO = 0x8828` -> FORMAT
            // 0xA, NUMBER_TYPE 0). An _SRGB attachment made Vulkan encode linear->sRGB on
            // store, but the guest's fragment values are ALREADY gamma-space (its atlases
            // are `nfmt = 0` UNORM and it composites in gamma space, exactly as a UNORM CB
            // does on real hardware). That spurious second encode washed every scene out.
            // With _UNORM the store is a raw write, so the guest's bytes survive untouched
            // and the present shader undoes the swapchain's encode instead.
    ...
    /// Create a cache-path buffer WITHOUT backing memory, for the suballocating pool to
    /// bind at an offset in a shared block ([`crate::buffer_pool`], task-223). Same usage
    /// flags the resource cache's copy path needs — a pooled buffer is bound as a vertex, index,
    /// uniform or storage buffer and is the destination of an upload memcpy.
    ///
    /// # Safety
    /// `self.device` must be live and owned by the caller.
    pub unsafe fn create_pool_buffer(&self, size: u64) -> vk::Buffer {
        let info = vk::BufferCreateInfo {
            s_type: vk::StructureType::BUFFER_CREATE_INFO,
            size: size.max(1),
        ...
        unsafe { self.device.create_buffer(&info, None) }.expect("pool buffer")
    }

    /// The first memory type in `type_bits` that is HOST_VISIBLE + HOST_COHERENT — the
    /// property pair the copy path needs so an upload is a plain memcpy through a persistent
    /// mapping with no flush.
    pub fn host_visible_memory_type(&self, type_bits: u32) -> u32 {
        let props = vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT;
        // SAFETY: `physical_device` belongs to `instance`, both live for the context.
        ...
        (0..mem_props.memory_type_count)
            ...
            .expect("a host-visible host-coherent memory type")
    }

    /// Allocate one suballocation block for the cache-buffer pool and map it for its whole
    /// lifetime. Leak-on-exit like the rest of the Vulkan state — a block is reused through
    /// the pool's free lists, never freed.
    ///
    /// Returns `None` when the allocation (or its mapping) fails — an oversized block derived
    /// from a large guest descriptor under memory pressure — so the pool degrades rather than
    /// aborting the display thread.
    ///
    /// # Safety
    /// `self.device` must be live and owned by the caller; `memory_type` must name a
    /// HOST_VISIBLE type.
    pub unsafe fn allocate_mapped_block(
        &self,
        size: u64,
        memory_type: u32,
        ...
        // SAFETY: the caller guarantees the device is live and `memory_type` host-visible.
        unsafe {
            let mem = match self.device.allocate_memory(&info, None) {
                Ok(m) => m,
                Err(e) => {
                    tracing::warn!(
                        "cache buffer pool block allocation of {size} bytes failed: {e}"
                    );
                    return None;
                }
            };
            ...
            {
                Ok(ptr) => Some((mem, ptr as *mut u8)),
                Err(e) => {
                    tracing::warn!("mapping cache buffer pool block failed: {e}");
    ...
    /// (doc-2 §C3/§C4). Usage covers `SAMPLED` (a pixel shader reads it through a combined
    ...
    ///
    /// Returns `Err` when the device cannot allocate the image (an oversized/degenerate guest
    /// texture descriptor, e.g. 16384×16384, under VRAM pressure) so the caller skips the
    /// resource rather than aborting the process.
    pub unsafe fn create_sampled_image(
        &self,
        w: u32,
        h: u32,
        format: vk::Format,
    ) -> VkResult<(vk::Image, vk::ImageView, vk::DeviceMemory)> {
        unsafe {
    ...
    /// Create an offscreen render-target image + view under the RT-as-texture path
    /// (doc-2 §8.5, task-56). Usage covers `COLOR_ATTACHMENT` (a draw renders into it),
    /// `SAMPLED` (a later draw reads it back through a combined image-sampler), and
    /// `TRANSFER_SRC` (a readback / PNG dump copies it out). This is the one-image-both-roles
    /// pattern already proven portable by the videoout `texture_image`
    /// ([`Self::create_texture`], vulkan.rs) — a single `R8G8B8A8_UNORM` image serving as both
    /// color attachment and sampled source, no `VK_IMAGE_CREATE_ALIAS_BIT`, no mutable-format
    /// views, `SAMPLE_1`. The image is created `UNDEFINED`; the caller tracks its layout across
    /// passes. The single COLOR-aspect 2D view serves both the attachment and sampled roles.
    ...
    ///
    /// Returns `Err` when the device cannot allocate the image (an oversized/degenerate guest
    /// render-target descriptor under VRAM pressure) so the caller skips the resource rather
    /// than aborting the process.
    pub unsafe fn create_render_target_image(
        &self,
        w: u32,
        h: u32,
        format: vk::Format,
    ...
    /// it `SHADER_READ_ONLY_OPTIMAL` for a combined image-sampler read (doc-2 §C3). A
    ...
    pub unsafe fn upload_image(&self, image: vk::Image, w: u32, h: u32, pixels: &[u8]) {
        ...
        unsafe {
            let (staging, staging_mem) = match Self::create_buffer(
                &self.instance,
                &self.device,
                self.physical_device,
                size,
                ...
                Ok(v) => v,
                Err(e) => {
                    // An oversized texture under memory pressure: skip the upload (the image
                    // stays undefined) rather than aborting the process.
                    tracing::warn!("upload_image: staging buffer allocation failed: {e}; skipping");
                    return;
                }
            };
    ...
    /// Create a `vk::Sampler` from the portable filter/address parameters (doc-2 §C4).
