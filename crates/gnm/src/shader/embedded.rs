//! `EmbeddedShaderProvider` (doc-2 §4, phase 3.5): embedded-id → hardcoded host
//! SPIR-V for the fixed fullscreen-quad VS + R/G-export PS pair (doc-1 §3.4). No
//! GCN — the emulator synthesizes host shaders from the ID and never sees a `.sb`
//! blob.
//!
//! This is the SINGLE route for every shader bind (doc-2 §4): the executor resolves
...
/// The firmware-embedded vertex-shader id for the fullscreen quad (doc-1 §3.4:
/// `sceGnmSetEmbeddedVsShader`, `shaderid 0`).
pub const EMBEDDED_VS_FULLSCREEN_QUAD: u32 = 0;
/// The firmware-embedded pixel-shader id that exports 32-bit R and G (doc-1 §3.4:
...
/// The hand-authored host SPIR-V for the two embedded shaders (doc-1 §3.4). Built
...
/// Vulkan-free boundary (doc-2 §1). The returned slice is 4-byte aligned.
...
impl ShaderProvider for EmbeddedShaderProvider {
    ...
    fn resolve(
        ...
        match *r {
            ...
            // (doc-2 §4). This MUST be `Ok(None)`, not `Err` — the `ChainProvider`
