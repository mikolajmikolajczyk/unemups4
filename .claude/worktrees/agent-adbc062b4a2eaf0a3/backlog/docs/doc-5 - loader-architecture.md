...
`doc-4` (GPU) and follows the same shape: current-state, layer boundaries, the
...
- **`linker.rs::load_executable(mgr, mem, &dyn ExecutableImage, name)`
  ...
  - Relocations applied inline; imports resolve eagerly via
    ...
    SYSCALL stub** (`MOV EAX, magic_id; SYSCALL; RET`, `doc-1` dec 2) that traps into
...
source, or a second module plugs into exactly one layer.** This mirrors `doc-4`'s
"trait in `ps4-core`, impl elsewhere, thin glue" discipline and the shadPS4 pipeline
(`Loader::Elf` → `Core::Module`/`DynamicInfo` → `Linker`/`SymbolsResolver`).
...
handful of future ones; a `match` arm per format is the right granularity (doc-4's
...
/// endian-swapped, Sony base64 — same algo as shadPS4 StringToNid / OpenOrbis).
...
for a named .prx, exactly as shadPS4 re-uses its static path); lazy binding (today's
...
If `doc-4`'s load-bearing seam is `GpuBackend`, the loader's is **`ParsedImage` +
...
task-27 AC #4), exactly like `doc-4` §7 keeps softgpu presenting at 60fps. Each step
...
**Over-engineering traps to avoid (doc-4 §b style):**
...
[shadPS4 Module Loading & Dynamic Linking (DeepWiki)](https://deepwiki.com/shadps4-emu/shadPS4/3.5-module-loading-and-dynamic-linking),
