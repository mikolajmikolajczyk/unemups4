...
- **GNM / GnmDriver** — the PS4's low-level graphics API (Sony's console equivalent of Vulkan). Phase 3/3.5 landed embedded-shader draw; real GCN draw is still in flight (phase 4).
- **Liverpool** — codename of the PS4's AMD APU/GPU; "Liverpool command processing" means consuming its PM4 command stream. The PM4 decoder is implemented; real GCN shader execution is not yet.
...
- **ShaderRef** — enum representing a bound shader before resolution. `ShaderRef::Embedded { stage, id }` names one of the built-in embedded SPIR-V shaders (phase 3.5). `ShaderRef::GcnBinary { addr, len, res }` carries the guest address, byte length, and RSRC-decoded register counts of a real `.sb` blob (phase 4). Lives in `crates/gnm/src/shader/source.rs`.
- **BackendCmd** — fire-and-forget command enum (`crates/core/src/gpu.rs`) sent from the GNM thread to the display/GPU thread over `GpuBackend::run_command_list`. Keeps `ps4-gnm` Vulkan-free; task-51/52 will extend it with buffer/texture upload variants.
- **wave64** — GCN's SIMD execution unit: 64 lanes running in lockstep, each lane holding one VGPR value. `EXEC` is a 64-bit mask controlling which lanes are active. The planned CPU wave interpreter (task-39) models this as `WaveState { vgprs[64], sgprs, exec, vcc, scc, … }`.
- **V# / T# / S#** — GCN hardware descriptor types encoding buffer/texture/sampler bindings as packed bitfields in SGPRs. V# (buffer descriptor, 128-bit) carries base address, stride, and size for vertex/constant fetch. T# (texture descriptor) and S# (sampler descriptor) carry image layout and filter state. Decode planned in task-45/55.
