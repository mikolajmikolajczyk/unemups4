//! GnmDriver HLE state (doc-2 §1): submit-queue bookkeeping, flip labels. Called
...
pub struct SubmitRange {
    ...
    pub flip: bool,
    /// Videoout handle the flip targets (`sceGnmSubmitAndFlipCommandBuffers` arg7).
    /// Meaningful only when [`flip`](Self::flip) is set; 0 otherwise.
    pub vo_handle: i32,
    /// Scanout buffer index to present (`sceGnmSubmitAndFlipCommandBuffers` arg8).
    /// Meaningful only when [`flip`](Self::flip) is set; 0 otherwise. For a
    /// double-buffered title this is the just-rendered buffer; presenting the wrong
    /// index scans out a stale/blank buffer.
    pub buf_idx: u32,
}
...
/// file (doc-2 §5/§C7): PS4 context/SH registers persist across submits, so the
...
pub struct GnmDriver {
    ...
    submit_batches: u64,
    state: GpuState,
    /// Guest-side host-pipeline cache (doc-2 §4, decision-7). Lives here, alongside the
    ...
    /// Guest-side resource cache (doc-2 §8): upload-on-use / invalidate-on-dirty for the
    ...
    /// The firmware-embedded shader provider (doc-2 §4). Owned here so the provider
    /// chain the executor threads persists across submits.
    embedded: EmbeddedShaderProvider,
    /// The GCN `.sb` → recompiled-SPIR-V provider (doc-2 §4, phase 4). Driver-owned so
...
impl GnmDriver {
    ...
    /// carrying the videoout handle and scanout buffer index the flip targets.
    pub fn submit_and_flip(
        &mut self,
        dcb_ptr: u64,
        dcb_size: u32,
        ccb_ptr: u64,
    ...
    /// Record an offset-index draw packet builder call (draws `index_count`
    /// indices starting at `index_offset` into the bound index buffer). No PM4
    /// is emitted yet.
    pub fn draw_index_offset(&mut self, index_offset: u32, index_count: u32) {
        let _ = (index_offset, index_count);
    }
    ...
    /// The submit-spanning GPU shadow state (doc-2 §5/§C7). The per-submit
    ...
    /// guest just freed/unmapped (doc-2 §8), appending the resulting
...
/// pattern (doc-2 §0: state in a well-known place, reached at runtime). The thin
...
mod tests {
    ...
    fn submit_and_flip_marks_flip() {
        let mut d = GnmDriver::new();
        d.submit_and_flip(0x4000, 512, 0, 0, 0, 1);
        let r = d.submissions()[0];
        assert!(r.flip);
        assert_eq!(r.dcb_ptr, 0x4000);
        assert_eq!(r.dcb_size, 512);
        assert_eq!(r.buf_idx, 1, "flip carries the guest's buffer index");
    }
