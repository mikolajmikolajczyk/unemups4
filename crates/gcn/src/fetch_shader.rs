//! Fetch-shader parsing (doc-2 §C4, phase 4): recover a VS's vertex-attribute
...
//! pull its vertex attributes. gnmx compiles it as the sequence below; the semantics
//! of each instruction are the AMD Sea Islands ISA (Liverpool = GCN2) and the opcode
//! encodings are pinned by `crate::opcodes`'s `gcn_opcodes_match_amd_oracle`:
//!
//! 1. one or more `s_load_dwordx4` (SMRD scalar load) load a buffer resource (a
//!    128-bit V#) from the descriptor set an SGPR pair points at, into a destination
//!    SGPR quad;
//! 2. one `buffer_load_format_*` per attribute (MUBUF) reads that V# with the `idxen`
//!    bit set, so the per-lane vertex index in `v0` selects the element — into an
//!    agreed destination VGPR block;
//! 3. `s_setpc_b64` (or `s_swappc_b64`) returns to the caller (SOP1).
//!
...
/// One vertex attribute recovered from a fetch shader (doc-2 §C4). Plain data —
...
/// The recovered vertex-attribute layout of a fetch shader (doc-2 §C4): the
...
/// (doc-2 §C4). Returns [`None`] when the stream is not the recognized fetch-shader
...
mod tests {
    ...
    #[test]
    fn parses_single_attribute_fetch_pos_shader() {
        // The fetch-CALL corpus callee `fetch_pos_vs` (used by inline_fetch_vs via
        // s_swappc_b64) fetches a single vec4 position attribute. It must parse to a
        // one-entry table — the minimal fetch shader the fetch-call path resolves.
        //   s_load_dwordx4 s[8:11], s[2:3], 0x0 -> V# in s8 from s2 @ 0
        //   buffer_load_format_xyzw v[4:7], v0, s[8:11] idxen -> v4, 4 comps, s8
        //   s_setpc_b64 s[0:1] -> return
        let p = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/corpus/fetch_pos_vs.code.bin");
        let bytes = std::fs::read(&p).unwrap_or_else(|e| panic!("read {}: {e}", p.display()));
        let code: Vec<u32> = bytes
            .chunks_exact(4)
            .map(|c| u32::from_le_bytes([c[0], c[1], c[2], c[3]]))
            .collect();
        let layout = parse_fetch_shader(&decode_all(&code)).expect("recognized fetch shader");
        assert_eq!(
            layout.attributes,
            vec![FetchAttribute {
                vsharp_sgpr: 8,
