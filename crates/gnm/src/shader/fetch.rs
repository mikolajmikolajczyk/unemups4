//! Fetch-shader → vertex-input state (doc-2 §C4, phase 4).
//!
...
/// attribute's destination VGPR carries (doc-2 §C4). Plain data — the vertex-input
...
/// The vertex-input state recovered from a fetch shader (doc-2 §C4): the resolved
...
/// Recover a fetch shader's vertex-input state (doc-2 §C4). Reads the fetch-shader
...
/// table by destination VGPR (doc-2 §C4). Each fetch attribute writes a destination
