...
Design:
- Bound-shader tracking: ps4-gnm::state — process-global BoundShaders{vs,ps: Option<ShaderRef>} (Mutex/OnceLock like driver()). HLE sceGnmSetEmbeddedVs/PsShader stubs (libs/libscegnmdriver) record Embedded{stage,id}. Executor IT_DRAW_INDEX_AUTO arm snapshots it, resolves BOTH stages through EmbeddedShaderProvider (doc-2 §4 single route).
