//! Guest-memory dirty-tracking seam (doc-4 §8.3).
//!
...
//!
//! Two impls exist (doc-4 §8.3): the real x86jit-backed one over the guest VM's
...
/// The dirty-tracking seam the resource cache polls (doc-4 §8.3).
///
...
/// cache re-uploads unconditionally (doc-4 §8.3). Selected when no VM is wired (headless)
