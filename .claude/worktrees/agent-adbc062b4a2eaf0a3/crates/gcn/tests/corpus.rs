...
/// `ShaderBinaryInfo::m_signature` magic (doc-3 §3.3).
const ORBSHDR_MAGIC: &[u8; 7] = b"OrbShdr";
/// Packed `ShaderBinaryInfo` header size in bytes (doc-3 §3.3).
const HEADER_SIZE: usize = 28;

/// `m_type` stage codes (doc-3 §3.3, matching `ps4_gnm::shader::sb::SbStage`).
...
/// the wrapped blob is a genuinely terminating program, not a marker payload. The
/// full "runs on shadPS4" acceptance check is deferred; this guards the corpus stays real GCN.
#[test]
