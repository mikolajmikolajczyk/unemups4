...
Build crates/gnm/src/shader/sb.rs (doc-2 §1 module-tree entry): parse "OrbShdr" magic, m_type (stage), m_length (24-bit GCN code size), hashes/crc, SRT flags, and VertexInputSemantic/VertexExportSemantic/PixelInputSemantic/PixelSemanticMapping tables (doc-1 §3.3; refs the OpenOrbis SDK ShaderBinaryInfo / .sb layout). Given ShaderRef::GcnBinary{addr} + &dyn VirtualMemoryManager, locate the shader-setup register block, return typed SbShader{stage,code_range,semantics}. Resolve the .sb address derivation from SPI_SHADER_PGM_LO/HI (addr>>8) for P4-09. Does NOT decode GCN, does NOT touch provider chain, rejects encrypted/garbage cleanly (NEVER decrypts — hard constraint). ps4-gnm stays Vulkan-free.
...
2. Parse ShaderBinaryInfo (28-byte OrbShdr header): magic 'OrbShdr', version, bitfields (m_pssl_or_cg/m_cached/m_type/m_source_type/m_length 24-bit), chunkUsageBaseOffsetInDW, numInputUsageSlots, SRT flags, hash0/1, crc32. Header sits AFTER GCN code; located by scanning for 'OrbShdr' magic (OpenOrbis SDK .sb layout).
