...
/// `ET_SCE_DYNEXEC`: a retail PS4 main executable. Its dynamic tables use
/// `DT_SCE_*` tags goblin cannot decode — routed to [`SceDynamic`].
const ET_SCE_DYNEXEC: u16 = 0xfe10;
/// `ET_SCE_DYNAMIC`: a retail PS4 shared object (.prx / .sprx). Same routing.
const ET_SCE_DYNAMIC: u16 = 0xfe18;
/// `PT_SCE_DYNLIBDATA`: presence of this program header marks an SCE image even
/// when `e_type` is a standard value.
...
impl ParsedImage {
    ...
    pub fn parse(container: Container) -> Result<ParsedImage, LoaderError> {
        ...
        let memory_size = segments
            .iter()
            .map(|s| (s.offset as usize) + s.data.len() + s.bss_size)
            .max()
            .unwrap_or(0);
        let sections = extract_sections(&elf);
        let tls = extract_tls(&elf, &raw)?;

        // PT_SCE_PROCPARAM (0x6100_0001): the SceKernelProcParam blob (heap/libc
        // config the runtime reads via sceKernelGetProcParam). It sits inside a
        // PT_LOAD, so its bytes are already mapped; we only need its module-relative
        // vaddr to hand back an absolute pointer once the module base is known.
        ...
        let (libraries, imports, exports, relocations) = if let Some(sce) = sce {
            (
                sce.libraries()?,
                sce.imports()?,
                sce.exports()?,
...
/// `PT_SCE_RELRO` (0x6100_0010): a loadable, relocated, then read-only segment.
...
mod tests {
    ...
    /// Fill a single ELF64 program header entry.
    fn put_phdr(
        ph: &mut [u8],
        p_type: u32,
        flags: u32,
