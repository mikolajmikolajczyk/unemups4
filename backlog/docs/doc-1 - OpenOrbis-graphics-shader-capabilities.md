...
PM4/GCN shader work in `decision-3`. This began as idea/research; the path it scoped
has since **landed** — the emulator now parses PM4, decodes GCN ISA, and
interprets/recompiles GCN shaders to render retail titles in-game (see `crates/gcn`,
`crates/gnm`; §6 for how the milestones this doc lays out map onto shipped code). Where the
GPU-approach research described the PS4 GPU stack in the abstract, **this doc grounds those
claims in what is physically on disk** (a full OpenOrbis checkout at `~/src/ps4labs/ps4sdk`)
and pins down the shader question at the "shaders arrive as GCN machine code" level.
...
imports (our HLE interception point)? `[DOCS]/[SPECULATION]` Yes — the PS4 GPU
architecture is: Gnm/gnmx builders (freegnm, or Sony's in retail games)
...
is the decision-3 "hook GnmDriver submits, parse PM4" surface, and task-20's
stub target.
...
`[DOCS]/[SPECULATION]` The *strong* form — "retail firmware stripped the
...
(§5). flatz's OpenGL-ES write-up
([psx-place](https://www.psx-place.com/threads/opengl-es-implementation-on-the-ps4-write-up-provided-by-flatz.21600/))
records `libScePigletv2VSH`'s compiler being removed on retail; treat "the general PSSL
...
`[DOCS]` Confirmed against the real `OrbShdr` blobs on disk — the game `.mgfxo` shaders
and captured GPU buffers from the console — plus OpenOrbis' headers:
...
This is the parser unemups4 needed before any GCN recompiler/interpreter could run a
guest shader (the metadata drives vertex-attribute fetch and VS→PS varying linkage) — now
implemented in `crates/gnm`, feeding the GCN interpreter/recompiler in `crates/gcn`.
...
and is not the target — raw Gnm/PM4 is (the path that landed).

---

## 6. Implications for task-22 and the PM4/GCN shader work

> **Status update:** the milestones this section lays out have since been built. The
> `libSceGnmDriver` submit path is hooked and PM4 is parsed; the embedded-shader draw and
> the arbitrary VS+PS (`.sb` → GCN) draw both work; retail titles render in-game through
> the full path (PM4 → GCN decode → interpret/recompile → Vulkan). The routing below is
> preserved as the original scoping rationale, not as open work.
...
because OpenOrbis ships no native Gnm sample (task-22 desc). This research
...
- task-22's acceptance criteria (issue a DrawIndexAuto-class PM4 stream; produce a PM4
  ...
  first corpus and matches the "boot-and-trace" (D1) + "present/sync" (D2) milestones
...
- If task-22 wants an actual **draw** packet in the stream (AC #2 says "DrawIndexAuto-class"),
  ...
  fully hand-written, and it gives the rasterizer something to actually rasterize later
...
- The `.sb` parser and GCN decoder are now **implemented in Rust** `[LANDED]` — `crates/gnm`
  parses the `OrbShdr` `.sb` layout and `crates/gcn` decodes GCN ISA, interprets it (the
  CPU-wave correctness oracle) and recompiles it to SPIR-V, both informed by the `.sb`
  layout and the SI ISA manual. This was the large, unavoidable piece; it is done.

---

## 7. Local toolchain inventory summary
