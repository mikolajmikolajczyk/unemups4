...
//! abstraction (doc-2 §1). Pure, Vulkan-FREE, headless-testable.
//!
//! Scaffolding only: the module tree from doc-2 §1 exists and compiles,
//! but no PM4 decode, executor, state, cache or shader logic is wired yet. Each
//! module grows in its own phase (doc-2 §7). The `ShaderProvider`/`ResourceCache`/
