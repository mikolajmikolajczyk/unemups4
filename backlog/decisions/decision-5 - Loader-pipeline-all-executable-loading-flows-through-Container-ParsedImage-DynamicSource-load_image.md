...
games don't bite us later." Design captured in `backlog/docs/doc-3` (loader
architecture, the companion to doc-2 for GPU).
...
- **L3 DynamicSource** — a trait producing the `Import`/`Export`/`Relocation` view,
  ...
  `MAP_BY_NID`, which is canonical (the OpenOrbis-documented salt + SHA-1 bit-slice NID hash);
...
(GPU pipeline). Full design + open questions: `backlog/docs/doc-3`.
