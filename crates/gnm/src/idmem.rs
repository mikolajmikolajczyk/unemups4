...
//! all-binds-through-a-provider route (doc-2 §4) without threading a heavyweight
...
///
/// task-138 LOCKDOWN: `pub(crate)` — the raw identity view is a gnm-internal seam only
/// (PM4 decoder hot path + executor label store). All out-of-crate guest memory access
/// goes through `ps4_core::{bounded_read,write_guest,guest_ptr}`.
pub(crate) struct IdentityMem;
...
impl VirtualMemoryManager for IdentityMem {
    ...
    /// IdentityMem models a fully-mapped identity space with no VMA gaps, so the
    /// "reject a sub-range that enters unmapped memory" contract of `write_bytes_ranged`
    /// is vacuously satisfied — the whole space is backed. Delegate to the default
    /// identity store (`write_bytes`) rather than inheriting the fail-loud
    /// `RANGED_WRITE_UNIMPLEMENTED` default, which the real backend (`VmMemoryManager`)
    /// overrides with a live VMA walk. Without this, routing a write through the
    /// `write_guest` seam over IdentityMem would skip the store (the DMA_DATA mem->mem
    /// writeback in `exec.rs`).
    fn write_bytes_ranged(&self, addr: u64, data: &[u8]) -> Result<(), &'static str> {
        VirtualMemoryManager::write_bytes(self, addr, data)
    }
...
/// `read_bytes_ranged` to snapshot an upload (doc-2 §8.2); those addresses are
...
///
/// task-138 LOCKDOWN: `pub(crate)` — only used in-crate (resource-cache uploads in
/// `exec.rs`).
